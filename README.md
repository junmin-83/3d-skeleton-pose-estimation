...
- `requirements.txt`는 `onnxruntime-gpu`(CUDA NVIDIA GPU)로 고정. **GPU가 없으면**
  `onnxruntime-gpu`를 주석 처리하고 `onnxruntime`을 활성화한 뒤 config의
  `detection.backend: cpu`로 바꾸세요(또는 데모용으로 `uv pip install onnxruntime`만 추가).
- **모델 캐시:** `rtmlib`은 첫 실행 시 RTMPose ONNX 모델을 받습니다. 저장 위치는 코드에서
  `TORCH_HOME=./models`로 지정되어 있어 **`./models/hub/checkpoints/`** 에 받습니다
  (`src/pose2d/rtmpose_detector.py`). `TORCH_HOME` 환경변수로 덮어쓸 수 있고, `models/`는
  `.gitignore` 처리됩니다. 상대경로이므로 **항상 프로젝트 루트에서 실행**하세요.
